...
then
  ...
  echo "Running ./script_H_charges.sh script in $PWD ..."
fi
###########################################################
...
do
  ...
  if [ -d "$(echo "$i" | cut -d':' -f1)" ]; then #if folder $i exists,  -d checks whether $jj directory exists or not
    ...
    do 
      ...
      #echo "$jjj"; 
      sed  -i "$jjj a $i" comp_final; 
    done < <(awk '{print $0}' comp3) #add protons
    ...
    do 
      #echo "$i"; 
      sed "/$i/d" aux_comp_final > comp_final2; 
      mv comp_final2 aux_comp_final; 
    done < <(awk '{print $0}' comp4) #delete protons
